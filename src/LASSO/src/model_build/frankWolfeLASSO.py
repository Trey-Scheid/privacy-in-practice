...
def ExponentialMechanism(A, y, l=1.0, tol=1e-4, epsilon=None, delta=1e-6, K=15000, trace=True, normalize=True, clip_sd=np.inf):
    ...
    if normalize and not epsilon is None:
        Ay, used_budget, maxes = private_normalize_data(Ay, min(epsilon*.2, .1), delta=delta)
        assert check_normalization(Ay)
        A, y = Ay[:,:-1], Ay[:, -1]
        epsilon -= used_budget
    ...
    for k in range(1, K+1):
        ...
        selected_idx = exponential_mechanism(scores, epsilon_t, l, L1)
    ...
    if not check_l_ball(x_new, l):
            print(f"Warning: Solution at iteration {k} is outside the L1 ball")
...
def exponential_mechanism(scores, epsilon, l, L1):
    """
    Select vertex using exponential mechanism
    ...
    if not epsilon is None:
        ...
        sensitivity = 2.0 * l * L1# For L1 normalized data
...
def private_normalize_data(data, epsilon, delta=1e-6):
    ...
    if not epsilon is None:
        ...
        # print("mech", sigma)
        # print("self", sensitivity * np.sqrt(2 * np.log(1.25 / delta)) / epsilon_prime)
    else:
        sigma=0
    ...
    normalized_data = data / np.maximum(1, noisy_maxes) #should max with 1?
...
def FW_NonPrivate(A, y, l=1.0, K=15000, tol=1e-4, trace=True, normalize=True, clip_sd=np.inf):
    if isinstance(A, pd.DataFrame):
        A = A.to_numpy()
    if isinstance(y, (pd.DataFrame, pd.Series)):
        y = y.to_numpy()
